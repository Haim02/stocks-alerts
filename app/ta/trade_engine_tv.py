...
# from app.core.schemas import TradePlan
...
def build_trade_plan_from_tv(
    ...
    *,
    ...
    ticker = str(tv.get("ticker", "")).upper().strip()
    signal = str(tv.get("signal", "")).upper().strip()
    interval = str(tv.get("interval", "")).upper().strip()  # כרגע לא בשימוש, אבל נשאר ל-notes/לוגיקה עתידית
    ...
    # --- Context checks (soft) ---
    if interval:
        notes.append(f"Interval: {interval}")
    if trend:
        notes.append(f"Trend: {trend}")
    if rsi is not None:
        notes.append(f"RSI: {round(rsi, 2)}")
    if volume is not None:
        notes.append(f"Volume: {round(volume, 2)}")
    ...
    # --- Entry ---
    entry = close
    entry_zone = {"low": round(entry * 0.995, 4), "high": round(entry * 1.005, 4)}  # ±0.5%

    # --- Stop logic (prefer support -> MA20 -> MA50) ---
    stop_candidates: List[float] = []
    ...
    tp2 = entry + (entry - stop) * max(rr_min + 1.0, 3.0)
