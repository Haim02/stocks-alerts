...
# from app.db.db import SessionLocal  # אם אצלך זה app.db.db import SessionLocal -> תחליף
# from app.db.email_send_log import EmailSendLog
...
# def _was_sent_recently(ticker: str, signal: str, hours: int = 12) -> bool:
...
#             .where(
#                 EmailSendLog.ticker == ticker,
#                 EmailSendLog.signal == signal,
#                 EmailSendLog.sent_at >= since,
#             )
#             .limit(1)
#         )
...
# def _log_sent(ticker: str, signal: str, subject: str | None) -> None:
#     db = SessionLocal()
#     try:
#         db.add(EmailSendLog(ticker=ticker, signal=signal, subject=subject))
#         db.commit()
#     finally:
#         db.close()
...
#             signal = ("PLAN_OK" if plan_ok else "SCAN")
#             cooldown_hours = int(os.getenv("EMAIL_COOLDOWN_HOURS", "12"))

#             if _was_sent_recently(ticker, signal, hours=cooldown_hours):
#                 print(f"[SP500_SCAN] skip {ticker} signal={signal} (sent < {cooldown_hours}h)")
#                 continue


#             send_email(subject=subject, html=html)
#             _log_sent(ticker, signal, subject)
#             sent += 1
#             print(f"[SP500_SCAN] email sent ticker={ticker} plan_ok={plan_ok}")
